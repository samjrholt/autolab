...
autolab is **an autonomous lab with provenance as its foundation**. Externally it is three layers — **Brain** (Claude Opus 4.7 as Planner and PolicyProvider, reading records and rendered figures), **Hands** (capability-named tools behind one MCP gateway, including LLM-backed *Interpretation Operations* like `pxrd_interpret`), **Ledger** (append-only hashed provenance with tags + annotations on every record). Internally it is five layers: Interface / Orchestration / Expertise / Tools / Provenance. The deliverable of a Campaign is goal-shaped (a material, a trend, a design); the ledger is the substrate that makes the autonomy trustworthy and the evidence compound. Every architectural choice must survive the substitution: *the record is the invariant, the agent is replaceable*. See [docs/design/2026-04-22-interpretations-and-metadata.md](docs/design/2026-04-22-interpretations-and-metadata.md) for the 2026-04-22 framing correction and the removal of the "Skill" abstraction.
