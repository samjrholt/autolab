...
- **sensor_shape_opt** - Minimal MaMMoS campaign demo for geometry optimisation. Registers a VM Resource, `mammos.sensor_material_at_T`, `mammos.sensor_shape_fom`, and a 2-step `WorkflowTemplate` where material outputs wire into the FOM step. `pixi run sensor-demo` creates queued Optuna and Claude/LLM campaigns with the same objective, bounds, budget, and workflow; each trial runs the full material -> FOM DAG and reports the FOM Record back to its planner. Use `pixi run sensor-demo -- --planner optuna` or `--planner claude` to create just one arm.
